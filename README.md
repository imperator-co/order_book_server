...
Notes:
- Entries are evicted on any terminal order status (`canceled`, `triggered`, `filled`, `rejected`, ...). On a snapshot install: if the hl-node dump yields trigger orders, the table is rebuilt from it wholesale (authoritative); if it yields none — true of current node versions, whose L4 dumps carry no trigger orders — the accumulated table is carried forward so a re-sync does not wipe it, and evictions cached during the fetch window still apply. **Consequence of barren dumps: the table only covers orders whose `"open"` status was seen while the server ran — stops placed before first startup are invisible until they churn.** Evictions are counted per status in `orderbook_untriggered_evictions_total{status}` — a novel status label after a node upgrade is the signal to re-check the eviction rule.
